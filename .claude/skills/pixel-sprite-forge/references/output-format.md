...
- **Borrar píxeles**: poné `0` en las posiciones de `data` que quieras
  transparentes.
- **Espejar**: usá `python3 scripts/img2sprite.py mirror NAME.json --out M.json`.

Después de editar el `.json`, re-renderizá para verificar:

```bash
python3 scripts/img2sprite.py render NAME.json --out preview.png
python3 scripts/img2sprite.py emit-ts NAME.json --out NAME.ts
```
